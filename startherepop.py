...
if __name__ == '__main__':
	...
	session='tigerp6'
	movie='duck8'
	(stim, spikes) = self.load_data(session=session, movie=movie) #self.channels defined here 
...
#	self.channels = spikes.shape[0]
	...
	glm_p=np.zeros([self.channels,self.channels-1])
	...
	for i in range(0,self.channels): # (10,self.channels,7):
		...
		otherspikes = np.double(spikes[np.arange(0,self.channels)!=i,:]>0)
		
		# estimate STA
		canvas4d, stother = self.sta(stimexp, otherspikes, spike) # STA sets self.sta_u ...
		fsign = 2*(self.sta_u.mean()>0)-1 # sign to flip
		sta_u[i,:] = fsign*self.sta_u.flatten()
		sta_v[i,:] = fsign*self.sta_v.flatten()
		
		# estimate GLM
		glm_u[i,:], glm_v[i,:], glm_p[i,:], glm_b[i] = self.learn(stimexp, spike, otherspikes)
		fsign = 2*(glm_u[i,:].mean()>0)-1 # sign to flip
		glm_u[i,:] = fsign * glm_u[i,:]
		glm_v[i,:] = fsign * glm_v[i,:]
		rsq[i,:]=self.plotkernels(glm_u[i,:], glm_v[i,:], glm_p[i,:], glm_b[i], stimexp, spike, otherspikes)
...
#	spike = np.double(spikes[i-1,:]>0) 
	spike = np.double(spikes[i,:]>0) 
	rsq[i,:]=self.plotkernels(glm_u[i,:], glm_v[i,:], glm_p[i,:], glm_b[i], stimexp, spike, otherspikes, plot=True)
	...
	for i in range(0,self.channels):
		...
		sta = frames.mean(0)
		cov = np.cov(frames.reshape(n, 24**2).T)
