...
def plot_pixel_sta(self, stim, spikes, session, movie):
	...
	for i in range(0,self.channels):
		...
		for k in range(1,6+1):
			...
			#print(E[:,-k].shape)
			plt.imshow( E[:,-k].reshape(stim.shape[1],stim.shape[2]) )
	# save image to disk. 
...
def load_ducks():
	...
	movie='duck30'
	(stim2, spikes2) = self.load_data(session=session, movie=movie)
	stim = np.vstack((stim,stim2))
	spikes = np.hstack((spikes,spikes2))
	stimexp = self.expand_stim(stim)
	return self, stim, stimexp, spikes
...
#	self.channels = spikes.shape[0]
	...
	i=24 # sample cell
#	spike = np.double(spikes[i-1,:]>0) 
	plot_kernels(self, stimexp, spikes, session, movie, glm_u, glm_v, glm_b, i)
	spike = np.double(spikes[i,:]>0) 
	...
	plot_pixel_sta(self, stim, spikes, session, movie)
