...
#		self.channels = 32 
	...
	def load_data(self, session='tigerp6', movie='duck8'):
		...
		path_to_movies = "/Users/danmossing/Documents/python/heise/"+d[movie]+"_down8_gauss.h5"
		path_to_spikes = "/Users/danmossing/Documents/python/heise/iSpikes_"+d[session]+".h5"

		h5 = tables.open_file(path_to_movies, mode = "r")
		stim=h5.get_node('/m')[:]
		h5.close()	
		...
		h5 = tables.open_file(path_to_spikes, mode = "r")
		#spikes=h5.get_node('/'+movie+'/'+session)[:]
		recording = h5.get_node('/data/'+movie)
		sites = [[child for child in group] for group in recording][0:-1]
		cells = [child for site in sites for child in site]
		bin_edges = [6.66*x for x in range(int(self.T*self.bin)+1)]
		spikes = np.array([np.histogram(x,bin_edges)[0] for x in cells])
		h5.close()
		self.channels = len(cells)
		if self.bin > 1:
			spikes = spikes[:,0:Tsub*self.bin].reshape(self.channels, Tsub, self.bin).sum(2)
	...
	def sta(self, stimexp, otherspikes, spike, plot=False):
		...
		self.sta_s = s[0]
		self.sta_v = -V[0:1,:]
		stother = otherspikes[triggers,:].mean(0)
		self.sta_p = stother
		#
	...
	def cost_store(self, x, args, debug=False):
		""" 
		x = u,v,p,b
		args = spike, otherspikes, stimexp
		...
		# unpack parameters
		stimexp, otherspikes, spike = args

		u = x[0:self.win**2]
		v = x[self.win**2:self.win**2+self.frame**2]
		p = x[self.win**2+self.frame**2:-1]
		b = x[-1]

		k = np.outer(u,v).flatten() # full filter is 64x81
		
		logf= np.dot(stimexp, k) + np.dot(otherspikes,p) + b # stimexp is 8*8x9*9, outer.flatten is (64, 81)
		f=np.exp(logf)
		...
		c = -(spike * logf - f * self.dt).mean(0) + self.lam*(u**2).sum(0) + self.lam*(v**2).sum(0) + self.lam*(p**2).sum(0)# maximize spike*logf overlap, minimize f
		if debug: 
			print("cost", c)
		...
		dlogfdp = np.dot(p, otherspikes[:,:,np.newaxis])
		dlogfdb = 1
		...
		cp = (spike[:, np.newaxis] * dlogfdp).mean(0) - (self.dt * dfdp).mean(0) - self.lam*2*p
		cb = (spike * dlogfdb).mean(0)                - (self.dt * dfdb).mean(0) 
		
		g = -np.hstack((cu, cv, cp, cb)) # 64+81+1 long
	...
	def learn(self, stimexp, spike, otherspikes):
		""" do the learning """	

		from scipy.optimize import fmin_l_bfgs_b as lbfgs
		otherno = otherspikes.shape[0]
		# initialize at STA
		if 1:
			...
			b=1*np.ones(1) # bias
			x0=np.vstack((u,v.T,np.random.randn(otherno,1),b)).flatten() / np.sqrt(self.sta_s) # package parameters
		else:
			x0 = 0.001 * np.random.randn(self.win**2+self.frame**2+otherno+1)

		#stimexp = self.expand_stim(stim) # build fourier representation, 3GB
		args = (stimexp[0:self.Ttrain,:], otherspikes[:,0:self.Ttrain].T, spike[0:self.Ttrain]) # leave a validation set
		
		# numerical sanity check
		if 1:
			epsi = 1e-6
			eps1  =np.zeros(self.win**2 +self.frame**2 +otherno +1); eps1[1]=epsi
			eps100=np.zeros(self.win**2 +self.frame**2 +otherno +1); eps100[self.win**2+1]=epsi
			eps145=np.zeros(self.win**2 +self.frame**2 +otherno +1); eps145[-1]=epsi
		...
		glm_v = x[self.win**2:self.win**2+self.frame**2]
		glm_p = x[self.win**2+self.frame**2:-1]
		glm_b = x[-1]
		
		return glm_u, glm_v, glm_p, glm_b
	
	def poissondraw(self,prediction):
		N = len(prediction)
		return np.random.rand(N)>np.exp(-prediction)

	def plotkernels(self, glm_u, glm_v, glm_p, glm_b, stimexp, spike, otherspikes, plot=False):
		...
		prediction = np.exp(np.dot(stimexp, k_matrix.flatten()) + np.dot(otherspikes.T, glm_p.flatten()) + glm_b)
		rsq = np.corrcoef(prediction[self.Ttrain:self.T], spike[self.Ttrain:self.T])[0,1]
		fake_spike = self.poissondraw(prediction)
		fake_rsq = np.corrcoef(prediction[self.Ttrain:self.T], fake_spike[self.Ttrain:self.T])[0,1]
		...
		print("---------------------------------")
		print("r-squared on training set={0}".format(rsq))
		print("---------------------------------")
		print("fake r-squared on training set={0}".format(fake_rsq))
		print("---------------------------------")

		return np.array([rsq, fake_rsq])
